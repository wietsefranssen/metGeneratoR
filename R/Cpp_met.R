...
cppFunction('NumericVector  solar_geom_c(float lat, int yday) {
  ...
  // int yday = 10;
        //    double PI = 3.141593;
            ...
            //int timesteps_per_day = 720;
            int timesteps_per_day = 24;
            ...
            int tinystepsperday, i;
            double coslat, sinlat, dh, decl, cosdecl, sindecl;
            double rad_fract_per_hour[timesteps_per_day];
            for (i =0; i < timesteps_per_day; i++) rad_fract_per_hour[i] = 0;
            ...
            /* Make chunks per hour */
            //    chunk_size = 2880 / timesteps_per_day;
            // ts_id = rep(c(1:timesteps_per_day),each=chunk_size)
            ...
            the timestep (J) */
            dir_beam_topa = (1368.0 + 45.5 * sin((2.0 * PI * (double) (yday - 1) / 365.25) + 1.7)) * dt;
            ...
            for (int j = 0; j < tinystepsperday; j++) {
            ...
            int istep;
            for (int tss = 0; tss < tinystepsperday; tss++) {
            istep = floor(tss / (tinystepsperday / timesteps_per_day));
            rad_fract_per_hour[istep] += tiny_rad_fract_1day[tss];
            result[istep] += tiny_rad_fract_1day[tss];
            }
...
# plot(solar_geom_c(-8.75, 10))
# sum(solar_geom_c(-88.75, 10))
# 
...
# seq(1, 360, 10)
# library(lattice)
# for (yday in seq(1, 360, 10)) {
#   filename<-print(sprintf("rplot_%003d.jpg", yday))
#   jpeg(filename)
#   
#   for (iy in 1:360) {
#     # iiy<-iy+180
...
#   }
#   for (hour in 1:24) {
#     # map_rad_frac[hour,] <-  map_rad_frac[hour,] / (sum(map_rad_frac)/(24*360))
#   }
#   image(map_rad_frac)
#   # print(levelplot(map_rad_frac, at = seq(0,.01,0.001)))
#   dev.off()
# }
# plot(map_rad_frac[,300])
# 
# ## dim = c(hour,lat)
# plot(map_rad_frac[,200])
# ## dim = c(yday,hour,lat)
# lines(hourly_rad_fract_data[1,,200])
# plot(map_rad_frac[,1])
# lines(hourly_rad_fract_data[1,,1])
